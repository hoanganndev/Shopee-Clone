...
const Header = () => {
  ...
  /**
   * - khi chuyển trang thì header chỉ bị re-render chứ kh unmount
   * - vì nằm chung trong mainLayout => react-router-dom nhận biết được
   * trừ trường hợp logout => registerLayout=> vào lại
   * - nên các query này sẽ không bị inActive => không bị gọi lại => không
   * cần phải set stale: Infinity(mãi mãi)
   */
            ...
            >
              <Link to='/' className='relative'>
                ...
                <span className='absolute top-[-5px] left-[17px] rounded-full bg-white px-[9px] py-[1px] text-xs text-orange '>
                  {purchasesInCart && purchasesInCart.length > 0 ? purchasesInCart.length : 0}
                </span>
              </Link>
            </Popover>
          </div>
