...
export default function Cart() {
  ...
  useEffect(() => {
    setExtendedPurchases((prev) => {
      ...
      console.log('extendedPurchaseObject', extendedPurchaseObject)
      return (
        purchasesInCart?.map((purchase) => {
          const isChoosePurchaseFromLocation = choosenPurchaseIdFromLocation === purchase._id
          return {
            ...purchase,
            disabled: false,
            checked:
              isChoosePurchaseFromLocation || Boolean(extendedPurchaseObject[purchase._id]?.checked)
          }
        }) || []
      )
    })
  ...
  // reset choosenProductId when F5 browser
  useEffect(() => {
    return () => {
  ...
  const handleCheck = (purchaseIndex: number) => (event: React.ChangeEvent<HTMLInputElement>) => {
    setExtendedPurchases(
      produce((draft) => {
        //draft đại diện cho extendedPurchases
